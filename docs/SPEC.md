...
**Counters** — the core relationship table. Maps an enemy defence team (per mode) to one or more counter teams, each with a tier, expected banner score, undersize count, and optional notes. This is the data that drives the lookup screen. The `Mode` column carries `5v5`, `3v3`, or `FLEET`; fleet counters are authored here exactly like squad counters, keyed by `Mode = FLEET`.

As of v2.8, two of these columns have refined meanings that own non-overlapping parts of a counter's value:
- **`Banner Score`** — the **full-squad, first-attempt, clean-clear** expected value, with any undersize premium removed. The companion `SCORING_REFERENCE.md` documents the per-mode ceilings (5v5 → 65, 3v3 → 57, fleet → 73) and the meaning ladder used to author these scores consistently.
- **`Undersize`** — a numeric **droppable-unit count** (previously a `Yes`/`No` flag): the maximum units the counter can drop from a full squad and still win cleanly, where `0` means field the full squad. Each unit dropped is worth **+1 banner** over the full clean clear, so the app reconstructs the undersize total as `Banner Score` + count. Any non-numeric value (a legacy `Yes`/`No`, or a blank) is read as `0`, so a partially-migrated sheet is always safe.

The Counters tab also carries a **`Reliability`** column (`High` / `Medium` / `Low`) recording how consistently a counter wins. It is authored for reference but **not currently consumed** by the Apps Script or the app; surfacing it (in the lookup card, or as an allocation-engine input) is a candidate future enhancement (see [§8](#8-roadmap)). Because the Apps Script reads the Counters tab by header name, this and any other unread column are silently ignored by the payload.
...
The user selects a mode (5v5 / 3v3 / Fleet) and a defence team, and sees the available counters. Each counter card displays the counter team name, tier (colour-coded), expected banners, undersize advice, and notes. A search box filters the defence-team list, and counters are sorted by status group, then tier, then by banner score.

From v2.8 the undersize line reflects the droppable-unit count (see [§4.1](#41-sheet-structure)): a counter that can shed units reads "drop up to N → X banners (+N)", showing the reconstructed best-case total; a counter that cannot reads "full squad".
...
**v2.8 — Undersize Optimiser (display)** · *Complete*
Surfaces undersize payoffs on the recommendation and lookup cards (see [§6.1](#61-counter-lookup), [§6.8](#68-allocation-engine)), driven by the `Undersize` droppable-count column and the rebased full-squad `Banner Score` (see [§4.1](#41-sheet-structure)). Leans on hand-authored catalogue values, so it needed no new scoring data and no fleet spot-check. Also carried the v2.8 fleet unit-count correction (8 → 7) into the points-to-win two-count model.

**Efficiency calculator — automatic undersize optimiser** · *Planned*
v2.8 *shows* the undersize payoff for a counter the player already has; the remaining piece is the engine *choosing* to undersize — e.g. the allocation engine actively preferring an undersized plan when it scores higher, or a final-battle "field exactly N units for maximum banners" recommendation that accounts for what the player can safely win with. The banner arithmetic is settled (the wiki fleet ladder confirmed +1 per unit dropped end to end); what remains is the winnability judgement — how far a given counter can safely undersize against a specific defence — which is exactly what the `Undersize` count now captures per counter but not yet per situation.

**Surface the Reliability column** · *Candidate*
The Counters tab authors a `Reliability` value (`High` / `Medium` / `Low`) that is not yet read by the Apps Script or app (see [§4.1](#41-sheet-structure)). Surfacing it — on the lookup and recommendation cards, and/or as a tiebreak or weighting in the allocation engine (a reliable Medium-banner counter may be preferable to a risky High-banner one on a must-win) — is a small, self-contained enhancement: one Apps Script field, a display line, and optionally an allocation objective tweak. Deferred until wanted; the data is already there.
