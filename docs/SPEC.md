...
**Persistence.** The board is versioned (schema 2), hydrated on boot before first paint, and defensively re-read on entry to the Round screen. It is cleared by Reset Round together with used teams and banner tracking (see [§3.4](#34-state--persistence)). A schema bump from 1 to 2 in v2.5 means any board created before fleet support (which carried no fleet team rows) is discarded on load and set up afresh.
